...
#			rem1, rem2 = group_overlapp(ca[cc].gnames,ca[cc2].gnames)	# which genes to remove to obtain the same set for both cancers
#			ca[cc].x = np.delete(ca[cc].x, rem1, axis=1)	# delete unique columns
#			ca[cc2].x = np.delete(ca[cc2].x, rem2, axis=1)
#			ca[cc].groups = np.delete(ca[cc].groups,rem1)
#			ca[cc2].groups = np.delete(ca[cc2].groups,rem2)
#			ca[cc].gnames = np.delete(ca[cc].gnames,rem1)
#			ca[cc2].gnames = np.delete(ca[cc2].gnames,rem2)
			ind_A, ind_B = group_overlapp(ca[cc].gnames,ca[cc2].gnames)	# which indices give shared elements
#			print('OBS: Removed '+str(ca[cc].x.shape[1]-len(ind_A))+', resp '+str(ca[cc2].x.shape[1]-len(ind_B))+' genes.')
...
class Cox:
	...
	def lp(self):		# first derivative wrt beta
		# auxiliary
#		q = self.X*self.theta[:,np.newaxis]   # matrix [theta_1*X_1, ..., theta_n*X_n] (same size as X)
#		Q = torch.flip(torch.cumsum(torch.flip(q,[0,1]),0),[0,1])  # cumulative sum over theta*X(flipped) # flip back to original ordering
#		temp3 = self.X[self.sset] - Q[self.tiess]/self.p[self.tiess][:, np.newaxis]
#		llp = -2/self.n_samples*torch.sum(temp3,axis=0) # loss fction derivative
#		
#		self.Xfl = torch.flip(self.X,[0,1])
#		self.thetafl = torch.squeeze(torch.flip(torch.unsqueeze(self.theta,0),[0,1]))
#		self.pfl = torch.squeeze(torch.flip(torch.unsqueeze(self.p,0),[0,1]))
#		qfl = self.Xfl*self.thetafl[:,np.newaxis]
#		self.ssetfl = torch.squeeze(torch.flip(torch.unsqueeze(self.sset,0),[0,1]))
#		self.tiessfl = np.flipud(self.tiess)
#		Qfl = torch.cumsum(qfl,0)
#		temp3fl = self.Xfl[self.ssetfl] - Qfl[self.tiessfl]/self.pfl[self.tiessfl][:, np.newaxis]
#		llp = -2/self.n_samples*torch.sum(temp3fl,axis=0) # loss fction derivative
...
class npCox:
	def __init__(self,c1,beta):
		...
		self.Z = c1.os_months 	# os_months_ordered
		self.beta = beta
		self.XB = np.dot(self.X,self.beta)#.flatten().astype(float)		# X*beta # make a one-dimensional vector
		self.theta = np.exp(self.XB)			# exp(X*beta)

		# resolve ties
		self.tie = ties(self.Z)

		# used in l & lp
		self.p = np.flipud(np.cumsum(np.flipud(self.theta)))	# cumulative sum of theta_tilde # flip back to the original ordering
		self.sset = np.array([i for i, x in enumerate(self.Y) if x])    # set of all event indices
		self.tiess = self.tie[self.sset]
				
	def l(self): # Cox negative partial log likelihood/loss function		
		temp3 = self.XB[self.sset] - np.log(self.p[self.tiess])
		ll = -2/self.n_samples*np.sum(temp3)	# vanilla loss function
		return ll
		
	def lp(self):		# first derivative wrt beta
		# auxiliary
		q = self.X*self.theta[:,np.newaxis]   # matrix [theta_1*X_1, ..., theta_n*X_n] (same size as X)
		Q = np.flipud(np.cumsum(np.flipud(q),axis=0)	)  # cumulative sum over theta*X(flipped) # flip back to original ordering
		temp3 = self.X[self.sset] - Q[self.tiess]/self.p[self.tiess][:, np.newaxis]
		llp = -2/self.n_samples*np.sum(temp3,axis=0) # loss fction derivative
		return llp
#################################################################################################################
def lasso(beta,glist):   # lasso regression
#	gnel = len(glist)	# nr of groups
##	grnorm = np.zeros(gnel)	# create an empty bucket for each group norm
##	grnorm = torch.zeros(gnel).to(device)	# create an empty bucket for each group norm
#	grnorm = torch.zeros(gnel)#,device=device)	# create an empty bucket for each group norm
##	grlen = [len(glist[jj]) for jj in range(gnel)]	# lengths of particular groups
##	grlen = torch.tensor([len(glist[jj]) for jj in range(gnel)])	# lengths of particular groups
##	grlen = torch.zeros(gnel, dtype = int, device = device)
#	weigh = torch.zeros(gnel)#, device = device)  # group weights
#	for jj in range(gnel):	# group norms
##		temp = len(glist[jj])	# lengths of particular groups
##		grlen[jj] = len(glist[jj])	 #temp
#		weigh[jj] = np.sqrt(len(glist[jj]))	# group weights = sqrt(length of the group)
##		for kk in range(temp): #range(grlen[jj]):
##			grnorm[jj]+=beta[glist[jj][kk]]**2	# sum squares within each group
#		grnorm[jj]=torch.sum(beta[np.array(glist[jj])]**2)	# sum squares within each group
##	grnorm = np.sqrt(grnorm)	# square root each sum
##	
##	weigh = np.sqrt(grlen)	# group weights = sqrt(length of the group)
##	reg1 = np.sum(weigh*grnorm)   # sum groups: group lasso regression
#	
#	grnorm = torch.sqrt(grnorm)	# square root each sum
##	grlen.clone().detach().requires_grad_(True)
##	torch.tensor(grlen,dtype=torch.float64)
##	weigh = torch.sqrt(grlen.clone().detach().type(torch.float64).requires_grad_(True)).to(device)	# group weights = sqrt(length of the group)
##	weigh = torch.sqrt(grlen.type(torch.float)).to(device)	# group weights = sqrt(length of the group)
##	weigh = torch.sqrt(grlen.type(torch.float))	# group weights = sqrt(length of the group)
#	reg1 = torch.sum(weigh*grnorm)   # sum groups: group lasso regression	
#	
##	reg2 = np.zeros(len(beta))
##	reg2 = torch.zeros(len(beta)).to(device)
#	reg2 = torch.zeros(len(beta))#, device = device)
#	for jj in range(gnel):
##		for kk in range(grlen[jj]):
##			if (grnorm[jj]!=0):
##				reg2[glist[jj][kk]] = weigh[jj]*beta[glist[jj][kk]]/grnorm[jj]
#		if (grnorm[jj]!=0):
#			reg2[np.array(glist[jj])] = weigh[jj]*beta[np.array(glist[jj])]/grnorm[jj]
...
def nplasso(beta,glist):   # lasso regression
	gnel = len(glist)	# nr of groups
	grnorm = np.zeros(gnel)	# create an empty bucket for each group norm
#	grlen = [len(glist[jj]) for jj in range(gnel)]	# lengths of particular groups
	grlen = np.zeros(gnel, dtype = int)#, device = device)
	
	for jj in range(gnel):	# group norms
		grlen[jj] = len(glist[jj])
#		for kk in range(grlen[jj]):
#			grnorm[jj]+=beta[glist[jj][kk]]**2	# sum squares within each group
		grnorm[jj]=np.sum(beta[np.array(glist[jj])]**2)	# sum squares within each group
	grnorm = np.sqrt(grnorm)	# square root each sum
...
#				reg2[glist[jj][kk]] = weigh[jj]*beta[glist[jj][kk]]/grnorm[jj]
	...
	return reg1, reg2
#####################################################################################################################
def multireg8(beta1,beta2,glist):   # group 1-multitasking
	gnel = len(glist)	# nr of groups
	grnorm = np.zeros(gnel)	# create an empty bucket for each group norm
	grlen = [len(glist[jj]) for jj in range(gnel)]	# lengths of particular groups
	diff = beta1-alpha*beta2  # beta difference
	for jj in range(gnel):	# group norms
		for kk in range(grlen[jj]):
			grnorm[jj]+=(diff[glist[jj][kk]])**2	# sum squares within each group
	grnorm = np.sqrt(grnorm)	# square root each sum
	
	weigh = np.sqrt(grlen)	# group weights = sqrt(length of the group)
	reg = np.sum(weigh*grnorm)   # sum groups: group lasso regression
	
	regp1 = np.zeros(len(beta1))
	regp2 = np.zeros(len(beta2))
	for jj in range(gnel):
		for kk in range(grlen[jj]):
			if (grnorm[jj]!=0):
				regp1[glist[jj][kk]] = weigh[jj]*(diff[glist[jj][kk]])/grnorm[jj]
				regp2[glist[jj][kk]] = -alpha*weigh[jj]*(diff[glist[jj][kk]])/grnorm[jj]
	return reg, regp1, regp2
#####################################################################################################################
def multireg(beta1,beta2,alpha): #multitask regularization: L2 norm
	lb = len(beta1)
#	alpha = 1
	if(np.any(beta1!=beta2)):
		betadifnorm = np.linalg.norm(beta1-alpha*beta2)	  # L2 norm of the difference between betas
		reg = betadifnorm 	# regularization
		regp1 = (beta1-alpha*beta2)/betadifnorm # derivative wrt beta1
		regp2 = -alpha*(beta1-alpha*beta2)/betadifnorm # derivative wrt beta2
	else:		# the values have already converged
		reg = 0
		regp1 = np.zeros(lb)
		regp2 = np.zeros(lb)
	return reg, regp1, regp2
#####################################################################################################################
def multireg7(beta1,beta2):#,dnew): #multitask regularization: L2 norm
	lb = len(beta1)
	alpha = 1
	if(np.any(beta1!=beta2)):
#		betadifnorm = np.linalg.norm((beta1-alpha*beta2)*dnew)#*np.sign(beta1)*np.sign(beta2))	  # L2 norm of the difference between betas
		betadifnorm = np.linalg.norm((beta1-alpha*beta2)*np.sign(beta1)*np.sign(beta2))	  # L2 norm of the difference between betas
		reg = betadifnorm 	# regularization
#		regp1 = (beta1-alpha*beta2)*dnew/betadifnorm # derivative wrt beta1
#		regp2 = -alpha*(beta1-alpha*beta2)*dnew/betadifnorm # derivative wrt beta2
		regp1 = (beta1-alpha*beta2)*((np.sign(beta1)*np.sign(beta2))**2)/betadifnorm # derivative wrt beta1
		regp2 = -alpha*(beta1-alpha*beta2)*((np.sign(beta1)*np.sign(beta2))**2)/betadifnorm # derivative wrt beta2
	else:		# the values have already converged
		reg = 0
		regp1 = np.zeros(lb)
		regp2 = np.zeros(lb)
	return reg, regp1, regp2
#####################################################################################################################
def multireg2(beta1,beta2): #multitask regularization: L1 norm
	lb = len(beta1)
	if(np.any(beta1!=beta2)):
		reg = np.linalg.norm(beta1-beta2,1) 	# regularization
		regp1 = np.sign(beta1-beta2) # derivative wrt beta1
		regp2 = -np.sign(beta1-beta2) # derivative wrt beta2
	else:		# the values have already converged
		reg = 0
		regp1 = np.zeros(lb)
		regp2 = np.zeros(lb)
	return reg, regp1, regp2
#####################################################################################################################
def multireg3(beta1,beta2,d): #multitask regularization: L2 norm od the identity function
	lb = len(beta1)
	alpha = 1
	if(np.any(beta1!=beta2)):
		betadifnorm = np.linalg.norm(np.multiply(d,beta1-alpha*beta2))	  # L2 norm of the difference between betas
		reg = betadifnorm 	# regularization
		regp1 = np.multiply(d**2,beta1-alpha*beta2)/betadifnorm # derivative wrt beta1
		regp2 = -alpha*np.multiply(d**2,beta1-alpha*beta2)/betadifnorm # derivative wrt beta2
	else:		# the values have already converged
		reg = 0
		regp1 = np.zeros(lb)
		regp2 = np.zeros(lb)
	return reg, regp1, regp2	
#####################################################################################################################
def multireg6(beta1,beta2,d): #multitask regularization: L1 norm
	lb = len(beta1)
	if(np.any(beta1!=beta2)):
		reg = np.linalg.norm(np.multiply(d,beta1-beta2),1) 	# regularization
		regp1 = np.multiply(d,np.sign(beta1-beta2)) # derivative wrt beta1
		regp2 = -np.multiply(d,np.sign(beta1-beta2)) # derivative wrt beta2
	else:		# the values have already converged
		reg = 0
		regp1 = np.zeros(lb)
		regp2 = np.zeros(lb)
	return reg, regp1, regp2
#####################################################################################################################
def multireg4(beta1,beta2): #multitask regularization: L2 norm
	lb = len(beta1)
	idx1 = (-np.abs(beta1)).argsort()[:(np.min([lb,3]))] # 3 largest elements in beta1
	idx2 = (-np.abs(beta2)).argsort()[:(np.min([lb,3]))] # 3 largest elements in beta2
	ind = np.unique(np.concatenate([idx1,idx2]))	# overlap
	beta1new = np.zeros(lb)
	beta1new[ind] = beta1[ind]
	beta2new = np.zeros(lb)
	beta2new[ind] = beta2[ind]
	alpha = 1
	if(np.any(beta1new!=beta2new)):
		betadifnorm = np.linalg.norm(beta1new-alpha*beta2new)	  # L2 norm of the difference between betas
		reg = betadifnorm 	# regularization
		regp1 = (beta1new-alpha*beta2new)/betadifnorm # derivative wrt beta1
		regp2 = -alpha*(beta1new-alpha*beta2new)/betadifnorm # derivative wrt beta2
	else:		# the values have already converged
		reg = 0
		regp1 = np.zeros(lb)
		regp2 = np.zeros(lb)
	return reg, regp1, regp2
#####################################################################################################################
def multireg5(beta1,beta2,d): #multitask regularization: L2 norm od the identity function
	lb = len(beta1)
	alpha = 1
	if(np.any(beta1!=beta2)):
		betadifnorm = np.linalg.norm(np.multiply(d,np.abs(beta1)-alpha*np.abs(beta2)))	  # L2 norm of the difference between betas
		reg = betadifnorm 	# regularization
		regp1 = np.multiply(np.multiply(d**2,beta1-alpha*beta2),np.sign(beta1))/betadifnorm # derivative wrt beta1
		regp2 = -alpha*np.multiply(np.multiply(d**2,beta1-alpha*beta2),np.sign(beta2))/betadifnorm # derivative wrt beta2
	else:		# the values have already converged
		reg = 0
		regp1 = np.zeros(lb)
		regp2 = np.zeros(lb)
	return reg, regp1, regp2
#####################################################################################################################
def multireg9(beta1,beta2): #multitask regularization: L2 norm
	lb = len(beta1)
#	alpha = 1
	beta1norm = np.linalg.norm(beta1,2)
	beta2norm = np.linalg.norm(beta2,2)
	if(beta1norm!=0 and beta2norm!=0):
#		betadif = beta1/beta1norm - beta2/beta2norm
		betadif = beta1 - beta2/beta2norm*beta1norm
		betadifnorm = np.linalg.norm(betadif,2)
#		betadifnorm = np.linalg.norm(beta1-alpha*beta2)	  # L2 norm of the difference between betas
		reg = betadifnorm 	# regularization
#		regp1 = (beta1-alpha*beta2)/betadifnorm # derivative wrt beta1
#		regp2 = -alpha*(beta1-alpha*beta2)/betadifnorm # derivative wrt beta2
#		regp1 = betadif/betadifnorm*(1/beta1norm - (beta1**2)/(beta1norm**3))
#		regp2 = betadif/betadifnorm*(-1/beta2norm + (beta2**2)/(beta2norm**3))
#		regp1 = betadif/betadifnorm*(1 - (beta1*beta2)/(beta1norm*beta2norm))
#		regp2 = betadif/betadifnorm*(-beta1norm/beta2norm*(1 - (beta2**2)/(beta2norm**2)))
		regp1 = 1/betadifnorm*(betadif - beta1/(beta1norm*beta2norm)*np.sum(beta2*betadif))
		regp2 = -1/betadifnorm*beta1norm/beta2norm*(betadif - beta2/(beta2norm**2)*np.sum(betadif*beta2))
	else:		# the values have already converged
		reg = 0
		regp1 = np.zeros(lb)
		regp2 = np.zeros(lb)
#	print(beta1norm/beta2norm)
	return reg, regp1, regp2

#aa,bb,cc = multireg9(beta_single[0,5,:],beta_single[0,8,:])
#print(aa)

#beta1 = np.array([0,1,2,3,4])
#beta2 = np.array([0,2,2,3,4])
##glist = [[1,2,3],[0,4]]
#a,b,c = multireg9(beta1,beta2)
...
#diff = np.zeros(5)
#for ii in range(5):
#	beta3 = np.array([0,2,2,3+h[ii],4])
##	beta13 = np.array([0,1+h[ii],2,3,4])
#	a3,b3,c3 = multireg9(beta1,beta3)
##	a3,b3,c3 = multireg9(beta13,beta2)
#	der2[ii] = (a3-a)/h[ii]
#	diff[ii] = np.abs(der2[ii]-c[3])
##	diff[ii] = np.abs(der2[ii]-b[1])
#	
...
#####################################################################################################################
def multireg10(beta1,beta2): #multitask regularization: L2 norm
	lb = len(beta1)
	beta1av = np.mean(beta1)
	beta2av = np.mean(beta2)
	if(beta1av!=0 and beta2av!=0):
		betadif = beta1/beta1av - beta2/beta2av
		betadifnorm = np.linalg.norm(betadif,2)
		reg = betadifnorm 	# regularization
		regp1 = 1/betadifnorm*(betadif/beta1av - np.mean(betadif*beta1)/(beta1av)**2)
		regp2 = -1/betadifnorm*(betadif/beta2av - np.mean(betadif*beta2)/(beta2av)**2)
	else:		# the values have already converged
		reg = 0
		regp1 = np.zeros(lb)
		regp2 = np.zeros(lb)
...
#####################################################################################################################
def multireg11(beta1,beta2): #multitask regularization: L2 norm
	lb = len(beta1)
	beta1av = np.mean(beta1)
	beta2av = np.mean(beta2)
	if(beta1av!=0 and beta2av!=0):
		betadif = (beta1/beta1av - beta2/beta2av)*np.sign(beta1)*np.sign(beta2)
		betadifnorm = np.linalg.norm(betadif,2)
		reg = betadifnorm 	# regularization
		regp1 = 1/betadifnorm*(betadif/beta1av - np.mean(betadif*beta1)/(beta1av)**2)
		regp2 = -1/betadifnorm*(betadif/beta2av - np.mean(betadif*beta2)/(beta2av)**2)
	...
	return reg, regp1, regp2
#################################################################################
def multireg20(beta1,beta2): #multitask regularization: L2 norm
	tol = 1e-4
	lb = len(beta1)
	beta1norm = torch.norm(beta1)
	beta2norm = torch.norm(beta2)
	if(beta1norm>tol and beta2norm>tol):
		betadif = (beta1/beta1norm - beta2/beta2norm)#*np.sign(beta1)*np.sign(beta2)
		betadifnorm = torch.norm(betadif)
		reg = betadifnorm 	# regularization
		regp1 = 1/betadifnorm*(betadif/beta1norm - beta1/(beta1norm**3)*torch.sum(betadif*beta1))
		regp2 = -1/betadifnorm*(betadif/beta2norm - beta2/(beta2norm**3)*torch.sum(betadif*beta2))
	else:		# the values have already converged
		reg = 0
		regp1 = torch.zeros(lb)
		regp2 = torch.zeros(lb)
	return reg, regp1, regp2
#####################################################################################################################
def multireg12(beta1,beta2,glist): #multitask regularization: L2 norm % PBBLY wrong anyway
	lb = len(beta1)
	betadif2 = np.zeros(lb)
	gnel = len(glist)	# nr of groups
	beta1av2 = np.zeros(gnel)	# create an empty bucket for each group norm
	beta2av2 = np.zeros(gnel)	# create an empty bucket for each group norm
	grlen = [len(glist[jj]) for jj in range(gnel)]	# lengths of particular groups
	for jj in range(gnel):	# group norms
		beta1av2[jj] = np.mean(beta1[glist[jj][:]])
		beta2av2[jj] = np.mean(beta2[glist[jj][:]])
		if (beta1av2[jj]!=0 and beta2av2[jj]!=0): # this removes the sign
			betadif2[glist[jj][:]] = beta1[glist[jj][:]]/beta1av2[jj] - beta2[glist[jj][:]]/beta2av2[jj];
	reg = np.linalg.norm(betadif2)

	regp12 = np.zeros(lb)
	regp22 = np.zeros(lb)
	for jj in range(gnel):
		if (beta1av2[jj]!=0 and beta2av2[jj]!=0): # this removes the sign
			sumk12 = np.sum(betadif2[glist[jj][:]]*beta1[glist[jj][:]])
			sumk22 = np.sum(betadif2[glist[jj][:]]*beta2[glist[jj][:]])
				
			regp12[glist[jj][:]] = 1/reg*(betadif2[glist[jj][:]]/beta1av2[jj] - 1/grlen[jj]*(1/beta1av2[jj]**2)*sumk12)
			regp22[glist[jj][:]] = -1/reg*(betadif2[glist[jj][:]]/beta2av2[jj] - 1/grlen[jj]*(1/beta2av2[jj]**2)*sumk22)
	return reg, regp12, regp22
#####################################################################################################################
def multireg13(beta1,beta2): #multitask regularization: L2 norm
	lb = len(beta1)
	beta1norm = np.linalg.norm(beta1,2)
	beta2norm = np.linalg.norm(beta2,2)
	if(beta1norm!=0 and beta2norm!=0):
		betadif = 1/2*(beta1 - beta2/beta2norm*beta1norm + beta2 - beta1/beta1norm*beta2norm)
		betadifnorm = np.linalg.norm(betadif,2)
		reg = betadifnorm 	# regularization
		regp1 = 1/2*1/betadifnorm*(betadif - beta1/(beta1norm*beta2norm)*np.sum(beta2*betadif)) - 1/2*1/betadifnorm*beta2norm/beta1norm*(betadif - beta1/(beta1norm**2)*np.sum(betadif*beta1))
		regp2 = 1/2*1/betadifnorm*(betadif - beta2/(beta2norm*beta1norm)*np.sum(beta1*betadif)) - 1/2*1/betadifnorm*beta1norm/beta2norm*(betadif - beta2/(beta2norm**2)*np.sum(betadif*beta2))
	else:		# the values have already converged
		reg = 0
		regp1 = np.zeros(lb)
		regp2 = np.zeros(lb)
	return reg, regp1, regp2
#####################################################################################################################
def multireg14(beta1,beta2): #multitask regularization: L2 norm
	lb = len(beta1)
	beta1norm = np.linalg.norm(beta1,2)
	beta2norm = np.linalg.norm(beta2,2)
	if(beta1norm > 1e-7 and beta2norm > 1e-7):
		betadif = 1/2*(beta1 - beta2/beta2norm*beta1norm + beta2 - beta1/beta1norm*beta2norm)*np.sign(beta1)*np.sign(beta2)
		betadifnorm = np.linalg.norm(betadif,2)
		reg = betadifnorm 	# regularization
		if(betadifnorm > 1e-7):
			regp1 = 1/2*1/betadifnorm*(betadif - beta1/(beta1norm*beta2norm)*np.sum(beta2*betadif)) - 1/2*1/betadifnorm*beta2norm/beta1norm*(betadif - beta1/(beta1norm**2)*np.sum(betadif*beta1))
	...
	return reg, regp1, regp2
#####################################################################################################################
def multireg15(beta1,beta2): #multitask regularization: L2 norm
	lb = len(beta1)
	beta1norm = np.linalg.norm(beta1,2)
	beta2norm = np.linalg.norm(beta2,2)
	k = 600;   # slope coefficient
	f1 = 2/np.pi*np.arctan(k*beta1);  # activation function
	f2 = 2/np.pi*np.arctan(k*beta2);
	f1d = k/((k*beta1)**2+1);  # derivative activation function
	f2d = k/((k*beta2)**2+1);  # derivative activation function
	if(beta1norm!=0 and beta2norm!=0):
		g = 1/2*(beta1 - beta2/beta2norm*beta1norm + beta2 - beta1/beta1norm*beta2norm) # auxiliary
		betadif = g*f1*f2
		betadifnorm = np.linalg.norm(betadif,2)
		reg = betadifnorm 	# regularization
		regp1 = 1/2*1/betadifnorm*(betadif*f1*f2 - beta1/(beta1norm*beta2norm)*np.sum(beta2*betadif*f1*f2)) \
				- 1/2*1/betadifnorm*beta2norm/beta1norm*(betadif*f1*f2 - beta1/(beta1norm**2)*np.sum(betadif*beta1*f1*f2)) \
				+ 1/2*1/betadifnorm*(np.sum(betadif*g*f1d*f2))
		regp2 = 1/2*1/betadifnorm*(betadif*f1*f2 - beta2/(beta2norm*beta1norm)*np.sum(beta1*betadif*f1*f2)) \
				- 1/2*1/betadifnorm*beta1norm/beta2norm*(betadif*f1*f2 - beta2/(beta2norm**2)*np.sum(betadif*beta2*f1*f2)) \
				+ 1/2*1/betadifnorm*(np.sum(betadif*g*f1*f2d))
	else:		# the values have already converged
		reg = 0
		regp1 = np.zeros(lb)
		regp2 = np.zeros(lb)
	return reg, regp1, regp2
#######################################################################################################################
def npmultireg16(beta1,beta2,glist): #multitask regularization: L2 norm
	lb = len(beta1)
	betadif2 = np.zeros(lb)
	gnel = len(glist)	# nr of groups
	grlen = [len(glist[jj]) for jj in range(gnel)]	# lengths of particular groups
	weigh = np.sqrt(grlen)	# group weights = sqrt(length of the group)
	betadif = np.zeros(gnel)
	beta1norm = np.zeros(gnel)	# create an empty bucket for each group norm
	beta2norm = np.zeros(gnel)	# create an empty bucket for each group norm
	for jj in range(gnel):	# group norms
		beta1norm[jj] = np.linalg.norm(beta1[glist[jj][:]])
		beta2norm[jj] = np.linalg.norm(beta2[glist[jj][:]])
		if (beta1norm[jj]>1e-7 and beta2norm[jj]>1e-7): # this removes the sign
			betadif2[glist[jj][:]] = 1/2*(beta1[glist[jj][:]] - beta2[glist[jj][:]]/beta2norm[jj]*beta1norm[jj] \
												  + beta2[glist[jj][:]] - beta1[glist[jj][:]]/beta1norm[jj]*beta2norm[jj]);
	...
	for jj in range(gnel):
		if (beta1norm[jj]>1e-7 and beta2norm[jj]>1e-7): # this removes the sign
			if reg>1e-7:
				regp12[glist[jj][:]] = 1/2*1/reg*weigh[jj]**2*(betadif2[glist[jj][:]] - beta1[glist[jj][:]]/(beta1norm[jj]*beta2norm[jj])*np.sum(beta2[glist[jj][:]]*betadif2[glist[jj][:]])) \
											- 1/2*1/reg*weigh[jj]**2*beta2norm[jj]/beta1norm[jj]*(betadif2[glist[jj][:]] - beta1[glist[jj][:]]/(beta1norm[jj]**2)*np.sum(betadif2[glist[jj][:]]*beta1[glist[jj][:]]))
				regp22[glist[jj][:]] = 1/2*1/reg*weigh[jj]**2*(betadif2[glist[jj][:]] - beta2[glist[jj][:]]/(beta2norm[jj]*beta1norm[jj])*np.sum(beta1[glist[jj][:]]*betadif2[glist[jj][:]])) \
											- 1/2*1/reg*weigh[jj]**2*beta1norm[jj]/beta2norm[jj]*(betadif2[glist[jj][:]] - beta2[glist[jj][:]]/(beta2norm[jj]**2)*np.sum(betadif2[glist[jj][:]]*beta2[glist[jj][:]]))
	return reg, regp12, regp22
#
#beta1 = np.array([0,1,2,3,-4])
#beta2 = np.array([1,2,-1,2,-1])
#glist = [[1,2,3],[0,4]]
#a,b,c = multireg16(beta1,beta2,glist)
#
...
#for ii in range(5):
#	beta13 = np.array([0,1+h[ii],2,3,-4])
##	beta3 = np.array([1,2,-1,2+h[ii],-1])
#	a3,b3,c3 = multireg16(beta13,beta2,glist)
##	a3,b3,c3 = multireg16(beta1,beta3,glist)
#	der2[ii] = (a3-a)/h[ii]
#	diff[ii] = np.abs(der2[ii]-b[1])
##	diff[ii] = np.abs(der2[ii]-c[3])
#	
...
###############################################################################
	## attempt to optimize
#def multireg16(beta1,beta2,glist): #multitask regularization: L2 norm
#	lb = len(beta1)
##	betadif2 = np.zeros(lb)
#	betadif2 = torch.zeros(lb)
#	gnel = len(glist)	# nr of groups
##	grlen = [len(glist[jj]) for jj in range(gnel)]	# lengths of particular groups
...
#	
#	glenmax = torch.max(grlen); # max length of a group
#	beta_gr1 = torch.zeros(gnel,int(glenmax))
#	beta_gr2 = torch.zeros(gnel,int(glenmax))
#	for jj in range(gnel):  # create a beta matrix
#		beta_gr1[jj,:int(grlen[jj])] = beta1[glist[jj][:]]
#		beta_gr2[jj,:int(grlen[jj])] = beta2[glist[jj][:]]	
#
##	for jj in range(gnel):	# group norms
###		beta1norm[jj] = np.linalg.norm(beta1[glist[jj][:]])
###		beta2norm[jj] = np.linalg.norm(beta2[glist[jj][:]])
...
##	reg = np.linalg.norm(weigh*betadif)  # weighted 2norm of 2 norms
#
#	beta1norm = torch.norm(beta_gr1, dim = 1)
#	beta2norm = torch.norm(beta_gr2, dim = 1)
##		if (beta1norm[jj]>1e-7 and beta2norm[jj]>1e-7): # this removes the sign
#	betadif2 = 1/2*(beta_gr1 - beta_gr2/beta2norm[:,np.newaxis]*beta1norm[:,np.newaxis] \
#                 + beta_gr2 - beta_gr1/beta1norm[:,np.newaxis]*beta2norm[:,np.newaxis]);
#	betadif = torch.norm(betadif2,dim=1)
#	reg = np.linalg.norm(weigh*betadif)  # weighted 2norm of 2 norms
#
##	regp12 = np.zeros(lb)
##	regp22 = np.zeros(lb)
##	regp12 = torch.zeros(lb)
##	regp22 = torch.zeros(lb)
##	for jj in range(gnel):
##		if (beta1norm[jj]>1e-7 and beta2norm[jj]>1e-7): # this removes the sign
##			if reg>1e-7:
###				regp12[glist[jj][:]] = 1/2*1/reg*weigh[jj]**2*(betadif2[glist[jj][:]] - beta1[glist[jj][:]]/(beta1norm[jj]*beta2norm[jj])*np.sum(beta2[glist[jj][:]]*betadif2[glist[jj][:]])) \
...
###				regp22[glist[jj][:]] = 1/2*1/reg*weigh[jj]**2*(betadif2[glist[jj][:]] - beta2[glist[jj][:]]/(beta2norm[jj]*beta1norm[jj])*np.sum(beta1[glist[jj][:]]*betadif2[glist[jj][:]])) \
###											- 1/2*1/reg*weigh[jj]**2*beta1norm[jj]/beta2norm[jj]*(betadif2[glist[jj][:]] - beta2[glist[jj][:]]/(beta2norm[jj]**2)*np.sum(betadif2[glist[jj][:]]*beta2[glist[jj][:]]))
##				regp12[glist[jj][:]] = 1/2*1/reg*weigh[jj]**2*(betadif2[glist[jj][:]] - beta1[glist[jj][:]]/(beta1norm[jj]*beta2norm[jj])*torch.sum(beta2[glist[jj][:]]*betadif2[glist[jj][:]])) \
##											- 1/2*1/reg*weigh[jj]**2*beta2norm[jj]/beta1norm[jj]*(betadif2[glist[jj][:]] - beta1[glist[jj][:]]/(beta1norm[jj]**2)*torch.sum(betadif2[glist[jj][:]]*beta1[glist[jj][:]]))
##				regp22[glist[jj][:]] = 1/2*1/reg*weigh[jj]**2*(betadif2[glist[jj][:]] - beta2[glist[jj][:]]/(beta2norm[jj]*beta1norm[jj])*torch.sum(beta1[glist[jj][:]]*betadif2[glist[jj][:]])) \
##											- 1/2*1/reg*weigh[jj]**2*beta1norm[jj]/beta2norm[jj]*(betadif2[glist[jj][:]] - beta2[glist[jj][:]]/(beta2norm[jj]**2)*torch.sum(betadif2[glist[jj][:]]*beta2[glist[jj][:]]))
#	if reg>1e-7:
#		regp12 = 1/2*1/reg*weigh[:,np.newaxis]**2*(betadif2 - beta_gr1/(beta1norm[:,np.newaxis]*beta2norm[:,np.newaxis])*torch.sum(beta_gr2*betadif2,dim=0)) \
#		       - 1/2*1/reg*weigh[:,np.newaxis]**2*beta2norm[:,np.newaxis]/beta1norm[:,np.newaxis]*(betadif2 - beta_gr1/(beta1norm[:,np.newaxis]**2)*torch.sum(betadif2*beta1))
##		regp22[glist[jj][:]] = 1/2*1/reg*weigh[jj]**2*(betadif2[glist[jj][:]] - beta2[glist[jj][:]]/(beta2norm[jj]*beta1norm[jj])*torch.sum(beta1[glist[jj][:]]*betadif2[glist[jj][:]])) \
##											- 1/2*1/reg*weigh[jj]**2*beta1norm[jj]/beta2norm[jj]*(betadif2[glist[jj][:]] - beta2[glist[jj][:]]/(beta2norm[jj]**2)*torch.sum(betadif2[glist[jj][:]]*beta2[glist[jj][:]]))
#		regp22 = 0
#	return reg, regp12, regp22
...
#	tol = 1e-8 #1e-3 #5e-4
#	beta_ind = (torch.abs(beta1)>tol)&(torch.abs(beta2)>tol)  # overlap indicator
#	beta1t = torch.zeros(len(beta1))
#	beta2t = torch.zeros(len(beta2))
#	beta1t[beta_ind] = beta1[beta_ind]
#	beta2t[beta_ind] = beta2[beta_ind]
#	
...
#											- 1/2*1/reg*weigh[jj]**2*beta1norm[jj]/beta2norm[jj]*(betadif2[glist[jj][:]] - beta2[glist[jj][:]]/(beta2norm[jj]**2)*np.sum(betadif2[glist[jj][:]]*beta2[glist[jj][:]]))
	...
	return reg, regp12, regp22
#######################################################################################################################
def multireg16b(beta1,beta2,glist): #multitask regularization: L2 norm
	tol = 1e-4
	lb = len(beta1)
#	betadif2 = np.zeros(lb)
...
#	weigh = np.sqrt(grlen)	# group weights = sqrt(length of the group)
	weigh = torch.sqrt(grlen)	# group weights = sqrt(length of the group)
#	betadif = np.zeros(gnel)
	betadif = torch.zeros(gnel, dtype=torch.float64)
...
#		beta2norm[jj] = np.linalg.norm(beta2[glist[jj][:]])
		...
		beta2norm[jj] = torch.norm(beta2[glist[jj][:]])
		if (beta1norm[jj]>tol and beta2norm[jj]>tol): # this removes the sign
			p1 = beta1[glist[jj][:]] - beta2[glist[jj][:]]/beta2norm[jj]*beta1norm[jj]
			p2 = beta2[glist[jj][:]] - beta1[glist[jj][:]]/beta1norm[jj]*beta2norm[jj]
...
#				regp22[glist[jj][:]] = 1/2*1/reg*weigh[jj]**2*(betadif2[glist[jj][:]] - beta2[glist[jj][:]]/(beta2norm[jj]*beta1norm[jj])*np.sum(beta1[glist[jj][:]]*betadif2[glist[jj][:]])) \
#											- 1/2*1/reg*weigh[jj]**2*beta1norm[jj]/beta2norm[jj]*(betadif2[glist[jj][:]] - beta2[glist[jj][:]]/(beta2norm[jj]**2)*np.sum(betadif2[glist[jj][:]]*beta2[glist[jj][:]]))
#				regp12[glist[jj][:]] = 1/2*1/reg*weigh[jj]**2*torch.sign(p1)*(betadif2[glist[jj][:]] - beta1[glist[jj][:]]/(beta1norm[jj]*beta2norm[jj])*torch.sum(beta2[glist[jj][:]]*betadif2[glist[jj][:]])) \
#											- 1/2*1/reg*weigh[jj]**2*beta2norm[jj]/beta1norm[jj]*torch.sign(p2)*(betadif2[glist[jj][:]] - beta1[glist[jj][:]]/(beta1norm[jj]**2)*torch.sum(betadif2[glist[jj][:]]*beta1[glist[jj][:]]))
#				regp22[glist[jj][:]] = 1/2*1/reg*weigh[jj]**2*torch.sign(p2)*(betadif2[glist[jj][:]] - beta2[glist[jj][:]]/(beta2norm[jj]*beta1norm[jj])*torch.sum(beta1[glist[jj][:]]*betadif2[glist[jj][:]])) \
#											- 1/2*1/reg*weigh[jj]**2*beta1norm[jj]/beta2norm[jj]*torch.sign(p1)*(betadif2[glist[jj][:]] - beta2[glist[jj][:]]/(beta2norm[jj]**2)*torch.sum(betadif2[glist[jj][:]]*beta2[glist[jj][:]]))
				regp12[glist[jj][:]] = 1/2*1/reg*weigh[jj]**2*(betadif2[glist[jj][:]]*torch.sign(p1) - beta1[glist[jj][:]]/(beta1norm[jj]*beta2norm[jj])*torch.sum(beta2[glist[jj][:]]*betadif2[glist[jj][:]]*torch.sign(p1))) \
											- 1/2*1/reg*weigh[jj]**2*beta2norm[jj]/beta1norm[jj]*(betadif2[glist[jj][:]]*torch.sign(p2) - beta1[glist[jj][:]]/(beta1norm[jj]**2)*torch.sum(betadif2[glist[jj][:]]*beta1[glist[jj][:]]*torch.sign(p2)))
				regp22[glist[jj][:]] = 1/2*1/reg*weigh[jj]**2*(betadif2[glist[jj][:]]*torch.sign(p2) - beta2[glist[jj][:]]/(beta2norm[jj]*beta1norm[jj])*torch.sum(beta1[glist[jj][:]]*betadif2[glist[jj][:]]*torch.sign(p2))) \
											- 1/2*1/reg*weigh[jj]**2*beta1norm[jj]/beta2norm[jj]*(betadif2[glist[jj][:]]*torch.sign(p1) - beta2[glist[jj][:]]/(beta2norm[jj]**2)*torch.sum(betadif2[glist[jj][:]]*beta2[glist[jj][:]]*torch.sign(p1)))

	return reg, regp12, regp22

#######################################################################################
def multireg17(beta1,beta2,glist): #multitask regularization: L2 norm
	lb = len(beta1)
	betadif2 = np.zeros(lb)
	gnel = len(glist)	# nr of groups
	grlen = [len(glist[jj]) for jj in range(gnel)]	# lengths of particular groups
...
#	grlen = [len(glist[jj]) for jj in range(gnel)]	# lengths of particular groups
	for jj in range(gnel):	# group norms
		beta1norm[jj] = np.linalg.norm(beta1[glist[jj][:]])
		beta2norm[jj] = np.linalg.norm(beta2[glist[jj][:]])
#		if (beta1norm[jj]!=0 and beta2norm[jj]!=0): # this removes the sign
		if (beta1norm[jj]>1e-7 and beta2norm[jj]>1e-7): # this removes the sign
			betadif2[glist[jj][:]] = 1/2*(beta1[glist[jj][:]] - beta2[glist[jj][:]]/beta2norm[jj]*beta1norm[jj]*np.sign(np.mean(beta1[glist[jj][:]]))*np.sign(np.mean(beta2[glist[jj][:]])) \
											   + beta2[glist[jj][:]] - beta1[glist[jj][:]]/beta1norm[jj]*beta2norm[jj]*np.sign(np.mean(beta1[glist[jj][:]]))*np.sign(np.mean(beta2[glist[jj][:]])));
#			betadif[jj] = np.linalg.norm(1/2*(beta1[glist[jj][:]] - beta2[glist[jj][:]]/beta2norm[jj]*beta1norm[jj] \
#							  + beta2[glist[jj][:]] - beta1[glist[jj][:]]/beta1norm[jj]*beta2norm[jj]))
		betadif[jj] = np.linalg.norm(betadif2[glist[jj][:]])
	reg = np.linalg.norm(weigh*betadif)  # weighted 2norm of 2 norms
...
#		if (beta1norm[jj]!=0 and beta2norm[jj]!=0): # this removes the sign
		if (beta1norm[jj]>1e-7 and beta2norm[jj]>1e-7): # this removes the sign
#		regp1 = 1/betadifnorm*(betadif - beta1/(beta1norm*beta2norm)*np.sum(beta2*betadif)) - 1/betadifnorm*beta2norm/beta1norm*(betadif - beta1/(beta1norm**2)*np.sum(betadif*beta1))
#		regp2 = 1/betadifnorm*(betadif - beta2/(beta2norm*beta1norm)*np.sum(beta1*betadif)) - 1/betadifnorm*beta1norm/beta2norm*(betadif - beta2/(beta2norm**2)*np.sum(betadif*beta2))
#			sumk12 = np.sum(betadif2[glist[jj][:]]*beta1[glist[jj][:]])
#			sumk22 = np.sum(betadif2[glist[jj][:]]*beta2[glist[jj][:]])
			if reg>1e-7:
				regp12[glist[jj][:]] = 1/2*1/reg*weigh[jj]**2*(betadif2[glist[jj][:]] - beta1[glist[jj][:]]/(beta1norm[jj]*beta2norm[jj])*np.sum(beta2[glist[jj][:]]*betadif2[glist[jj][:]]*np.sign(np.mean(beta1[glist[jj][:]]))*np.sign(np.mean(beta2[glist[jj][:]])))) \
											- 1/2*1/reg*weigh[jj]**2*beta2norm[jj]/beta1norm[jj]*(betadif2[glist[jj][:]] - beta1[glist[jj][:]]/(beta1norm[jj]**2)*np.sum(betadif2[glist[jj][:]]*beta1[glist[jj][:]]*np.sign(np.mean(beta1[glist[jj][:]]))*np.sign(np.mean(beta2[glist[jj][:]]))))
				regp22[glist[jj][:]] = 1/2*1/reg*weigh[jj]**2*(betadif2[glist[jj][:]] - beta2[glist[jj][:]]/(beta2norm[jj]*beta1norm[jj])*np.sum(beta1[glist[jj][:]]*betadif2[glist[jj][:]]*np.sign(np.mean(beta1[glist[jj][:]]))*np.sign(np.mean(beta2[glist[jj][:]])))) \
											- 1/2*1/reg*weigh[jj]**2*beta1norm[jj]/beta2norm[jj]*(betadif2[glist[jj][:]] - beta2[glist[jj][:]]/(beta2norm[jj]**2)*np.sum(betadif2[glist[jj][:]]*beta2[glist[jj][:]]*np.sign(np.mean(beta1[glist[jj][:]]))*np.sign(np.mean(beta2[glist[jj][:]]))))
	return reg, regp12, regp22
######################################################################################
def multireg18(beta1,beta2): #cos distance
	
	tol = 1e-10 #5e-4
	beta_ind = (torch.abs(beta1)>tol)&(torch.abs(beta2)>tol)  # overlap indicator
#	beta1t = torch.zeros(len(beta1), dtype=torch.float64)
#	beta2t = torch.zeros(len(beta2), dtype=torch.float64)
	beta1t = torch.zeros(len(beta1))
	beta2t = torch.zeros(len(beta2))
	beta1t[beta_ind] = beta1[beta_ind]
	beta2t[beta_ind] = beta2[beta_ind]
	
	beta1norm = torch.norm(beta1t)	# norm
	beta2norm = torch.norm(beta2t)	# norm
	
	cos = torch.matmul(beta1t,beta2t)/beta1norm/beta2norm
	reg = torch.abs(1-cos)

	regp12 = -1/beta2norm*(beta2t/beta1norm - beta1t*torch.matmul(beta1t,beta2t)/beta1norm**3)*torch.sign(1-cos)
	regp22 = -1/beta1norm*(beta1t/beta2norm - beta2t*torch.matmul(beta1t,beta2t)/beta2norm**3)*torch.sign(1-cos)

	return reg, regp12, regp22
################################################################################
######################################################################################
def multireg21(beta1,beta2,glist): #cos distance # multireg18 group-wise
	tol = 5e-4 #5e-4
	...
	for jj in range(gnel):	# group norms
		...
		beta2norm[jj] = torch.norm(beta2[glist[jj][:]])
		if (beta1norm[jj]>tol and beta2norm[jj]>tol): # this removes the sign
			cos[jj] = torch.matmul(beta1[glist[jj][:]],beta2[glist[jj][:]])/beta1norm[jj]/beta2norm[jj]
			betadif[jj] = torch.abs(1- cos[jj])
	reg = np.linalg.norm(weigh*betadif)  # weighted 2norm of 2 norms
	...
	for jj in range(gnel):
		if (beta1norm[jj]>tol and beta2norm[jj]>tol): # this removes the sign
			if reg>tol:
				regp12[glist[jj][:]] = 1/reg*weigh[jj]**2*betadif[jj]*(-1/beta2norm[jj]*(beta2[glist[jj][:]]/beta1norm[jj] - beta1[glist[jj][:]]*torch.matmul(beta1[glist[jj][:]],beta2[glist[jj][:]])/beta1norm[jj]**3)*torch.sign(1-cos[jj]))
				regp22[glist[jj][:]] = 1/reg*weigh[jj]**2*betadif[jj]*(-1/beta1norm[jj]*(beta1[glist[jj][:]]/beta2norm[jj] - beta2[glist[jj][:]]*torch.matmul(beta1[glist[jj][:]],beta2[glist[jj][:]])/beta2norm[jj]**3)*torch.sign(1-cos[jj]))
...
#glist = [[1,2,3],[0,4]]
#a,b,c = multireg21(beta1,beta2,glist)
#
##beta12 = np.array([0,1,2,3,4])
##h = [0.2,0.1,0.05,0.025,0.0175]
#h = torch.zeros(5)
...
#	a3,b3,c3 = multireg21(beta13,beta2,glist)
##	a3,b3,c3 = multireg21(beta1,beta3,glist)
#	der2[ii] = (a3-a)/h[ii]
...
##################################################################################
def multireg19(beta1,beta2): #cos distance
...
#	beta1t[beta_ind] = beta1[beta_ind]
#	beta2t[beta_ind] = beta2[beta_ind]
	beta1t = copy.copy(beta1)
	beta2t = copy.copy(beta2)
	
	beta1norm = torch.norm(beta1t)	# norm
	beta2norm = torch.norm(beta2t)	# norm
	
	cos = torch.matmul(beta1t,beta2t)/beta1norm/beta2norm
	reg = torch.sqrt(torch.abs(1-cos**2))

	regp1cos = 1/beta2norm*(beta2t/beta1norm - beta1t*torch.matmul(beta1t,beta2t)/beta1norm**3)*torch.sign(1-cos)
	regp2cos = 1/beta1norm*(beta1t/beta2norm - beta2t*torch.matmul(beta1t,beta2t)/beta2norm**3)*torch.sign(1-cos)
	
	regp12 = -cos/reg*regp1cos
	regp22 = -cos/reg*regp2cos
...
#	beta3 = np.array([1,2,-1,2+h[ii],-1],dtype = 'float')
#	beta3 = torch.from_numpy(beta3)
##	a3,b3,c3 = multireg19(beta13,beta2)
#	a3,b3,c3 = multireg19(beta1,beta3)
#	der2[ii] = (a3-a)/h[ii]
##	diff[ii] = torch.abs(der2[ii]-b[1])
#	diff[ii] = torch.abs(der2[ii]-c[3])
#	
...
#plt.show()	
######################################################################################
def multireg22(beta1,beta2,glist): # monster 16 split
	tol = 1e-3 #5e-4
...
################################################################################
#################################################################################
def multireg23(beta1,beta2,glist): #multireg 20 group-wise
	tol = 5e-4 #5e-4
	...
	for jj in range(gnel):	# group norms
		...
		beta2norm[jj] = torch.norm(beta2[glist[jj][:]])
		if (beta1norm[jj]>tol and beta2norm[jj]>tol): # this removes the sign
			betadif2[glist[jj][:]] = beta1[glist[jj][:]]/beta1norm[jj] - beta2[glist[jj][:]]/beta2norm[jj]
		betadif[jj] = torch.norm(betadif2[glist[jj][:]])
	reg = np.linalg.norm(weigh*betadif)  # weighted 2norm of 2 norms
...
#				regp22[glist[jj][:]] = 1/reg*weigh[jj]**2*betadif[jj]*(1/p2norm[jj]*(p2[glist[jj][:]] - beta2[glist[jj][:]]/beta2norm[jj]/beta1norm[jj]*torch.sum(p2[glist[jj][:]]*beta1[glist[jj][:]])) \
#											- 1/p1norm[jj]*(beta1norm[jj]/beta2norm[jj]*p1[glist[jj][:]] - beta2[glist[jj][:]]*beta1norm[jj]/(beta2norm[jj]**3)*torch.sum(p1[glist[jj][:]]*beta2[glist[jj][:]])))
				regp12[glist[jj][:]] = 1/reg*weigh[jj]**2*betadif[jj]*1/betadif[jj]*(betadif2[glist[jj][:]]/beta1norm[jj] - beta1[glist[jj][:]]/(beta1norm[jj]**3)*torch.sum(betadif2[glist[jj][:]]*beta1[glist[jj][:]]))
				regp22[glist[jj][:]] = -1/reg*weigh[jj]**2*betadif[jj]*1/betadif[jj]*(betadif2[glist[jj][:]]/beta2norm[jj] - beta2[glist[jj][:]]/(beta2norm[jj]**3)*torch.sum(betadif2[glist[jj][:]]*beta2[glist[jj][:]]))
...
#	print('    k         ||ss||              max(lp(r))       '  )
	...
	while (incr>tol) and (kk<kmax) and (loss_delta>tol):
		...
		glist, gunq = group_list(c1_copy.groups)	# groups in lists, unique groups
		Cfunc = npCox(c1_copy,beta_it)
		lreg,lpreg = nplasso(beta_it,glist)	# compute lasso regeression
		loss = Cfunc.l() + lam*lreg  # compute loss & gradient
		der = Cfunc.lp() + lam*lpreg
		...
		# 	update gradient step
			gamma = np.abs(np.dot(betaold-betaoldold,der-derold))/(np.linalg.norm(der-derold,2))**2	# Barzilai-Borwein method)
		ss = gamma*der  # absolute increment
		beta_it -= ss

		# check for new zeros
		indzero = []
		for jj in range(len(glist)):
			if (np.all(np.abs(beta_it[glist[jj]])<tol2)):
				indzero.extend(glist[jj])	# extend = append multiple elements	
				
		# update the zero set
		nnrange = [i for i in range(len(nonzero)) if i not in indzero]    # set of all event indices
	
		if(len(nnrange) == 0): # break in case all coefficients are 0
			nonzero = []
			nonzerogr = []
			beta_fin = np.zeros(c1_copy.n_genes)#,dtype='float32')
			c1_copy.groups = []
#			print(kk, loss, len(nonzero))
			break
			
		# update beta and increment
		incr = np.linalg.norm(ss)/np.linalg.norm(beta_it)	# relative increment
		indy = nonzero[indzero]
		beta_fin[indy] = np.zeros(len(indzero))#,dtype='float32')	# save the full nonsparse beta
		# update nonzero indices
		nonzero = nonzero[nnrange]	# update nonzero set
		beta_it = beta_it[nnrange]
		c1_copy.groups = c1_copy.groups[nnrange]
		beta_fin[nonzero] = beta_it  # update nonzeros
		c1_copy.x = c1_copy.x[:,nnrange]
		
		betaoldold = betaold[nnrange] #copy.deepcopy(betaold[nnrange])
		betaold = copy.deepcopy(beta_it)
		derold = copy.deepcopy(der[nnrange])
		
		score_train = np.dot(c1.x,beta_fin)#.flatten().astype(float)
		cind_train[kk] = c_index(score_train,c1.os_status,c1.os_months)
		beta_v[kk,:] = beta_fin
		loss_v[kk] = loss
...
#		print(kk,loss, len(nonzero))
		...
		if(kk == kmax):
			Warning('GD did not converge for some values of lambda.');
		
#	print(kk,loss, len(nonzero))
	glist, nonzerogr = group_list(c1_copy.groups)	# groups in lists, unique groups
...
for cancers_type in cancers:
	...
	nk = 2#2#3#1#3 #3 # 3 # nk CV repeats
